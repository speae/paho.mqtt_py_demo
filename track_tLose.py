...
# python3 track.py --source 2 --yolo_weight yolov5s.pt --show-vid --class 0
...
def detect(opt):
    ...
    for frame_idx, (path, img, im0s, vid_cap, depth_img, depth_im0s, depth_data) in enumerate(dataset):
        ...
        targetId_dic = {} # 리스트초기화 : 현 프레임에서의 모든 {id:거리}가 들어가는 딕셔너리
                    ...
                    for j, (output, conf) in enumerate(zip(outputs, confs)): 
                        ...
                        if 0.01 < target_distacne < 7.0:
                            ...
                            if following_pers == 0:
                                ...
                                chaseIdList.append(following_pers)  
                            ...
                            if following_pers == id:
                                ...
                                save_targetdis = distance_val
                                # 타겟 추적성공시 서치실패시 플래그초기화
                                noSearchCountFlag = 0

                            # following_pers가 추적이 안되고 있으면 (딕셔너리 키안에 id 추가가 안되있으면)
                            if not following_pers in targetId_dic:
                                noSearchCountFlag += 1
                                # 3프레임이상 없었으면
                                if noSearchCountFlag > 4:
                                    # 추격리스트중에 맨앞부터 id가 있는놈을 쫒음
                                    for ch_id in chaseIdList:
                                        if ch_id == id:
                                            following_pers = id
                                            break
                                # 10프레임간 없었으면
                                # 비슷한 거리에있는 새로운 타겟을 찾아 추가
                                if 40 > noSearchCountFlag > 8:
                                    print("타겟 8프레임간 잃어버림")
                                    # 화면 리스트내에 있는 비슷한거리(+-3cm)의 타겟을 찾아 추가,변경
                                    for targId, targ_dis in targetId_dic.items():
                                        if 2.0 + save_targetdis > targ_dis > save_targetdis - 2.0 :
                                            # 타겟변경할것을 chseIdList에 기록
                                            chaseIdList.append(targId)
                                            print("#####변경된 타겟 : {}##### 추격중인 id리스트 = {}".format(targId, chaseIdList))
                            #print("화면에 표시된 모든 id 딕셔너리 : {}".format(targetId_dic))
                            #print("추격중인 id 리스트 : {}".format(chaseIdList))
